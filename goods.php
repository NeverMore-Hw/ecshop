...
if (!$smarty->is_cached('good.dwt', $cache_id))
{
...
$smarty->assign("attr",$properties['pro']['商品属性']);
$smarty->assign("typel",$properties["spe"][211]["values"]);

//echo "<pre>";
//print_r($properties['spe']);

$smarty->display('good.dwt',$cache_id);
...
function get_linked_goods($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['goods_middle']  = get_image_path($row['goods_id'], $row['goods_middle'], true);
...
{
    $sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_thumb, g.goods_middle, g.goods_img, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$key]['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb'], true);
         $arr[$key]['goods_middle'] = get_image_path($row['goods_id'], $row['goods_middle'], true);
...
function get_package_goods_list($goods_id)
{
    ...
    foreach ($res as $tempkey => $value)
    {
        ...
        $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, p.goods_attr, g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, g.goods_middle, IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price
        ...
        foreach($goods_res as $key => $val)
        {
            ...
            $goods_res[$key]['goods_middle']  = get_image_path($val['goods_id'], $val['goods_middle'], true);
