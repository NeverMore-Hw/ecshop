...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, g.goods_middle, SUM(og.goods_number) as goods_number ' .
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
        ...
        $arr[$i]['middle'] = get_image_path($arr[$i]['goods_id'], $arr[$i]['goods_middle'],true);
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_middle, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['middle']        = get_image_path($row['goods_id'], $row['goods_middle'], true);
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_middle, goods_img, b.brand_name, " .
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        $row['goods_middle'] = get_image_path($goods_id, $row['goods_middle'], true);       
        return $row;
    }
    else
    {
...
function get_same_attribute_goods($attr)
{
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            /* 查找符合条件的商品 */
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_middle, g.goods_img, g.shop_price AS org_price, ' .
                        "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
            ...
            foreach ($res AS $row)
            {
                ...
                $lnk[$key]['goods'][$row['goods_id']]['goods_middle']     = (empty($row['goods_middle'])) ? $GLOBALS['_CFG']['no_picture'] : $row['goods_middle'];
...
{
    $sql = 'SELECT img_id, img_url, thumb_url, middle_url,img_desc' .
    ...
    foreach($row as $key => $gallery_img)
    {
        ...
        $row[$key]['middle_url'] = get_image_path($goods_id, $gallery_img['middle_url'], true, 'gallery');
    }
    return $row;
}
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_middle, g.goods_img ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '')
{
    $sql =  'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_middle, g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_middle, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$temp_index]['goods_middle']       = get_image_path($row['goods_id'], $row['goods_middle'], true);
