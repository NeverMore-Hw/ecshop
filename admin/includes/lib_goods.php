    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if ($proc_thumb)
            {
                ...
                $middle_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key], $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                $middle_url = is_string($middle_url) ? $middle_url : '';
            }
            ...
            $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb');
             $middle_url = reformat_image_name('gallery_middle', $goods_id, $middle_url, 'middle');

            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, middle_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$middle_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
        ...
        elseif (!empty($image_urls[$key]) && ($image_urls[$key] != $GLOBALS['_LANG']['img_file']) && ($image_urls[$key] != 'http://') && copy(trim($image_urls[$key]), ROOT_PATH . 'temp/' . basename($image_urls[$key])))
        {
            ...
            if ($proc_thumb)
            {
                ...
                $thumb_url = is_string($thumb_url) ? $thumb_url : '';
                $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb');
                $middle_url = $GLOBALS['image']->make_thumb($down_img, $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                $middle_url = is_string($middle_url) ? $middle_url : '';
                $middle_url = reformat_image_name('gallery_middle', $goods_id, $middle_url, 'middle');
            }
            ...
            if (!$proc_thumb)
            {
                ...
                $middle_url = htmlspecialchars($image_url);
            }         
...
function reformat_image_name($type, $goods_id, $source_img, $position='')        
{
    ...
    if (!make_dir(ROOT_PATH.$dir.'/'.$sub_dir.'/middle_img'))
    {
        return false;
    }
    ...
    switch($type)
    {
        ...
        case 'goods_thumb':
            ...
            break;
         case 'goods_middle':
            $img_name = $goods_id . '_middle_G_' . $rand_name;
            break;
        case 'gallery':
        ...
        case 'gallery_thumb':
            ...
            break;
         case 'gallery_middle': 
            $img_name = $goods_id . '_middle_P_' . $rand_name;          
            break;
    }
    if ($position == 'source')
    {
    ...
    {       
        ...
        if (move_image_file(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/middle_img/'.$img_name.$img_ext))
        {
        
            return $dir.'/'.$sub_dir.'/middle_img/'.$img_name.$img_ext;
        }
    }
    else
    {
