        ...
        if(!empty($good_name))
        {
            ...
            if ($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none')
            {
                ...
                if ($image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']))
                {
                    ...
                    if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0)
                    {
                        $gallery_middle = $image->make_thumb('../'. $gallery_middle , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
            ...
            if (!empty($original_img))
            {
                ...
                {
                    $gallery_middle = $original_img;
                }
            }
        
          
            $sql = 'INSERT INTO ' . $ecs->table('goods') . "(goods_name, goods_sn, goods_number, cat_id, brand_id, goods_brief, shop_price, market_price, goods_img, goods_thumb, goods_middle, original_img,add_time, last_update,
                   is_best, is_new, is_hot)" .
                   "VALUES('$good_name', '$goods_sn', '$good_number', '$cat_id', '$brand_id', '$good_brief', '$good_price'," .
                   " '$market_price', '$goods_img', '$goods_thumb', '$gallery_middle', '$original_img','" . gmtime() . "', '". gmtime() . "', '$is_best', '$is_new', '$is_hot')";
                   ...
                   if (isset($img))
                   {
                       ...
                       "VALUES ('$good_id', '$gallery_img', '', '$gallery_thumb','$gallery_middle', '$img')";
                       $db->query($sql);
                   }
