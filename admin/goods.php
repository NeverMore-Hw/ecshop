...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 10) && !empty($goods['original_img']))
        {
            ...
            $goods['goods_middle'] = get_image_path($_REQUEST['goods_id'], $goods['goods_middle'], true);
        }
        ...
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0))
        {
            foreach ($img_list as $key => $gallery_img)
            {
                ...
                $gallery_img[$key]['middle_url'] = get_image_path($gallery_img['goods_id'], $gallery_img['img_original'], true, 'gallery');
            }
        }
        else
        {
            foreach ($img_list as $key => $gallery_img)
            {
                ...
                $gallery_img[$key]['middle_url'] = '../' . (empty($gallery_img['middle_url']) ? $gallery_img['img_url'] : $gallery_img['middle_url']);
            }
        }
    }
    ...
    $smarty->assign('middle_width', $_CFG['image_width']);
    $smarty->assign('middle_height', $_CFG['image_height']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
        ...
        if (isset($_FILES['goods_thumb']))
        {
         ...
         // 商品缩略图
        if (isset($_FILES['goods_middle']))
        {
            if ($_FILES['goods_middle']['error'] == 0)
            {
                if (!$image->check_img_type($_FILES['goods_middle']['type']))
                {
                    sys_msg($_LANG['invalid_goods_middle'], 1, array(), false);
                }
            }
            elseif ($_FILES['goods_middle']['error'] == 1)
            {
                sys_msg(sprintf($_LANG['goods_middle_too_big'], $php_maxsize), 1, array(), false);
            }
            elseif ($_FILES['goods_thumb']['error'] == 2)
            {
                sys_msg(sprintf($_LANG['goods_middle_too_big'], $htm_maxsize), 1, array(), false);
            }
        }
    ...
    else
    {
        ...
        if (isset($_FILES['goods_thumb']))
        {
          ...
          if (isset($_FILES['goods_middle']))
        {
            if ($_FILES['goods_middle']['tmp_name'] != 'none')
            {
                if (!$image->check_img_type($_FILES['goods_middle']['type']))
                {
                    sys_msg($_LANG['invalid_goods_middle'], 1, array(), false);
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        {
            /* 删除原来的图片文件 */
            $sql = "SELECT goods_thumb, goods_middle, goods_img, original_img " .
            ...
            if ($row['goods_thumb'] != '' && is_file('../' . $row['goods_thumb']))
            {
              ...
              if ($row['goods_middle'] != '' && is_file('../' . $row['goods_middle']))
            {
                @unlink('../' . $row['goods_middle']);
            }
            ...
            if ($proc_middle === false)
            {
                get_image_path($_REQUEST[goods_id], $row['goods_img'], false, 'goods', true);
                get_image_path($_REQUEST[goods_id], $row['goods_middle'], true, 'goods', true);
            }
        }
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if (empty($is_url_goods_img))
            {
                ...
                if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0)
                {
                    $goods_middle = $image->make_thumb('../'. $goods_img , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                    if ($goods_img === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
            ...
            if ($_CFG['auto_generate_gallery'])
            {
                if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
                {
                 ...
                 if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0)
                {
                    $gallery_thumb = $image->make_thumb('../' . $img, $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                    if ($gallery_middle === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
                }
                  if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0)
                {
                    $gallery_middle = $image->make_thumb('../' . $img, $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                    if ($gallery_middle === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
        ...
        isset($_FILES['goods_thumb']['tmp_name']) &&$_FILES['goods_thumb']['tmp_name'] != 'none')
    {
        ...
        $goods_middle = $image->upload_image($_FILES['goods_middle']);
        if ($goods_thumb === false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
         if ($goods_middle === false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
    }
    else
    {
        ...
        if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img))
        {
            ...
            if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0)
            {
                $goods_middle = $image->make_thumb('../' . $original_img, $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                if ($goods_middle === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
    ...
    $goods_middle = (empty($goods_middle) && !empty($_POST['goods_middle_url']) && goods_parse_url($_POST['goods_middle_url'])) ? htmlspecialchars(trim($_POST['goods_middle_url'])) : $goods_middle;
    $goods_middle = (empty($goods_middle) && isset($_POST['auto_middle']))? $goods_img : $goods_middle;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, goods_middle, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$goods_middle', '$original_img', ".
    ...
    $goods_middle = reformat_image_name('goods_middle', $goods_id, $goods_middle, 'middle');
    if ($goods_img !== false)
    {
    ...
    if ($goods_thumb !== false)
    {
     ...
     if ($goods_middle !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_middle = '$goods_middle' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $gallery_middle = reformat_image_name('gallery_middle', $goods_id, $gallery_middle, 'middle');
      
        
        $sql = "INSERT INTO " . $ecs->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, middle_url, img_original) " .
                "VALUES ('$goods_id', '$gallery_img', '', '$gallery_thumb', '$gallery_middle', '$img')";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb,goods_middle " .
    ...
    if (!empty($goods['goods_thumb']))
    {
     ...
     if (!empty($goods['goods_middle']))
    {
        @unlink('../' . $goods['goods_middle']);
    }
    ...
    /* 删除商品相册 */
    $sql = "SELECT img_url, thumb_url, middle_url, img_original " .
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        if (!empty($row['middle_url']))
        {
            @unlink('../' . $row['middle_url']);
        }
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    if ($row['thumb_url'] != '' && is_file('../' . $row['thumb_url']))
    {
     ...
     if ($row['middle_url'] != '' && is_file('../' . $row['middle_url']))
    {
        @unlink('../' . $row['middle_url']);
    }
